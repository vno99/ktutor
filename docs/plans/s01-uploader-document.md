...
15. [x] **Tests unitaires** :
    ...
    - Fixture `conftest.py` : PDF synthétique de 3 pages (maths : dérivées), PNG avec texte tapé, PNG « manuscrit simulé », fichier trop gros (25 Mo généré en `tmp_path`), fichier invalide (`.exe`). *AC couverts : tous (1 par test).*
