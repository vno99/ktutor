...
- **Pas de modification d'`Attempt`** : la Tâche 8 câblée sur l'endpoint existant — si l'endpoint n'existe pas (s04/s07 pas encore shipped), la Tâche 8 se réduit à la doc et un test « skipped » ; le câblage réel attendra les stories s04/s07.
...
3. **Tâche 8 (cache invalidation on new Attempt) peut être bloquante** si l'endpoint `Attempt` n'existe pas encore. Le plan **détecte** en Tâche 8 (« STOP et reporter »), ne crée **pas** l'endpoint dans cette story (hors-scope, s04/s07 owner). Le câblage `invalidate_dashboard` attendra ces stories. **Si bloquante** : split en s16a (sans invalidation explicite, TTL 5 min suffit) et s16b (câblage invalidation après merge s04/s07). Recommandation : ne pas blocker, accepter le TTL 5 min pour la POC.
