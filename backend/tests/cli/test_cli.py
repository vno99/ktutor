...
def stubbed_qcm_grader(monkeypatch: pytest.MonkeyPatch):
    """Patch ``_build_grader_service`` AND ``_build_progressive_service`` so
    the CLI uses our stubs (s08 wires both — the s04 grader feeds a
    verdict into the s08 service via a ``grade_callback``)."""
    holder: dict[str, _StubQcmGrader] = {}
    progressive_holder: dict[str, _StubProgressiveService] = {}

    def _factory(behavior: str = "ok"):
        stub = _StubQcmGrader(behavior=behavior)
        holder["svc"] = stub
        progressive = _StubProgressiveService(behavior="ok")
        progressive_holder["svc"] = progressive
        monkeypatch.setattr("app.cli._build_grader_service", lambda: stub)
        monkeypatch.setattr("app.cli._build_progressive_service", lambda: progressive)
        return stub
...
class TestSubmitQcm:
    ...
    def test_submit_qcm_json_output_is_valid(self, stubbed_qcm_grader) -> None:
        ...
        # s08 — the ``attempt_id`` is now generated by the progressive
        # service (the s04 grader's attempt_id is the input, the
        # service's own attempt_id is the output). The s04 fields
        # ``correct_count`` / ``total`` are no longer part of the JSON
        # output (the progressive service exposes the s08 contract
        # instead).
        assert payload["attempt_id"] == "aaaaaaaa-aaaa-aaaa-aaaa-aaaaaaaaaaaa"
        # The s08 contract — correction_level, hints, etc.
        assert payload["correction_level"] == "full"
        assert payload["solution"] == "solution complete"
        assert payload["bonus_points"] == 2
...
def stubbed_text_grader(monkeypatch: pytest.MonkeyPatch):
    """Patch ``_build_text_grader_service`` AND ``_build_progressive_service``
    so the CLI uses our stubs (s08 wires both)."""
...
class TestSubmitText:
    ...
    def test_submit_text_json_output_is_valid(
        ...
        # s08 — the progressive service generates the new attempt_id.
        assert payload["attempt_id"] == "aaaaaaaa-aaaa-aaaa-aaaa-aaaaaaaaaaaa"
        assert payload["correction_level"] == "full"
        assert payload["solution"] == "solution complete"
        assert payload["bonus_points"] == 2
...
# ---------------------------------------------------------------------------
# s08 — progressive correction wiring on submit-qcm / submit-text
# ---------------------------------------------------------------------------


class _StubProgressiveService:
    """Acts as ``ProgressiveCorrectionService`` for the CLI.

    The CLI calls ``evaluate(pseudo, exercise_id, grade_callback)`` and
    inspects the returned ``CorrectionResult``. This stub returns a
    scripted result and records the grade_callback it received.
    """
    ...
    def evaluate(self, pseudo: str, exercise_id: str, grade_callback: Any):
        from app.services.correction.progressive import (
            CorrectionResult,
            ProgressiveCorrectionError,
        )

        # Mirror the real service: invoke the grade_callback BEFORE
        # returning a verdict. The CLI relies on the callback to wire
        # the underlying grader (s04 / s07). Errors raised by the
        # callback are propagated so the CLI exits with the right
        # code (mimic the real service's behaviour for closed /
        # cross_tenant / invalid_exercise).
        is_success, feedback = grade_callback(None, pseudo)
        self.calls.append((pseudo, exercise_id, (is_success, feedback)))
        if self.behavior == "ok":
            return CorrectionResult(
                ...
                hints=[] if is_success else ["indice 1", "indice 2"],
                next_steps=None if is_success else "relis le cours",
                solution="solution complete" if is_success else None,
                detailed_correction=None,
                common_mistakes=None,
                bonus_points=2 if is_success else 0,
            )
        if self.behavior == "closed":
            raise ProgressiveCorrectionError("closed", "Exercice ferme apres 3 tentatives.")
        if self.behavior == "cross_tenant":
            raise ProgressiveCorrectionError("cross_tenant", "Exercise introuvable pour 'bob'.")
        if self.behavior == "invalid_exercise":
            raise ProgressiveCorrectionError(
                "invalid_exercise", "Exercise de type 'flashcards' refuse."
            )
        if self.behavior == "full_after_attempts":
            return CorrectionResult(
                is_success=False,
                feedback=feedback,
                correction_level="full_after_attempts",
                attempt_number=3,
                attempt_id=uuid.UUID("cccccccc-cccc-cccc-cccc-cccccccccccc"),
                hints=[],
                next_steps=None,
        ...
        if self.behavior == "partial_attempt_2":
            return CorrectionResult(
                is_success=False,
                feedback=feedback,
                correction_level="partial_attempt_2",
                attempt_number=2,
                attempt_id=uuid.UUID("bbbbbbbb-bbbb-bbbb-bbbb-bbbbbbbbbbbb"),
                hints=["erreur identifiee"],
                next_steps="concentre-toi sur l'etape 3",
                solution=None,
                detailed_correction=None,
                common_mistakes=None,
                bonus_points=0,
...
@pytest.fixture()
def stubbed_progressive_service(monkeypatch: pytest.MonkeyPatch):
    """Patch ``_build_progressive_service`` so the CLI uses our stub."""
    holder: dict[str, _StubProgressiveService] = {}

    def _factory(behavior: str = "ok"):
        stub = _StubProgressiveService(behavior=behavior)
        holder["svc"] = stub
        monkeypatch.setattr(
            "app.cli._build_progressive_service", lambda: stub
        )
        return stub
...
class TestSubmitQcmProgressive:
    """s08 — submit-qcm is extended to call the progressive service."""

    def test_submit_qcm_first_try_failure_returns_partial_and_hints(
        self, stubbed_qcm_grader, stubbed_progressive_service
    ) -> None:
        """AC1 — first failure → exit 0, JSON carries correction_level='partial'."""
        qcm_factory, _ = stubbed_qcm_grader
        prog_factory, holder = stubbed_progressive_service
        qcm_factory("wrong")
        prog_factory("ok")
        result = runner.invoke(
            app,
        ...
        assert result.exit_code == EXIT_OK, result.stdout
        # The progressive service was called and the grade_callback
        # it received was the (s04) QCM grader's verdict.
        assert len(holder["svc"].calls) == 1
        # The QCM grader reported failure (2/3 correctes) and the
        # service translated that into (is_success=False, ...).
        assert holder["svc"].calls[0][2] == (False, "2/3 réponses correctes.")
        # The JSON output carries correction_level=partial + hints.
        text = result.stdout
        start = text.find("{")
        depth = 0
        end = -1
        for i in range(start, len(text)):
            ...
            if ch == "{":
                ...
                if depth == 0:
                    ...
                    break
        payload = json.loads(text[start:end])
        assert payload["correction_level"] == "partial"
        assert payload["hints"] == ["indice 1", "indice 2"]
        assert payload["next_steps"] == "relis le cours"
        assert payload["is_success"] is False
        assert payload["attempt_number"] == 1

    def test_submit_qcm_first_try_success_returns_full(
        self, stubbed_qcm_grader, stubbed_progressive_service
    ) -> None:
        """AC4 — first-try success → correction_level='full' + solution + bonus."""
        qcm_factory, _ = stubbed_qcm_grader
        prog_factory, _ = stubbed_progressive_service
        qcm_factory("ok")
        prog_factory("ok")
        result = runner.invoke(
            app,
    ...
    def test_submit_qcm_third_try_failure_returns_full_after_attempts(
        self, stubbed_qcm_grader, stubbed_progressive_service
    ) -> None:
        """AC3 — third failure → correction_level='full_after_attempts'."""
        qcm_factory, _ = stubbed_qcm_grader
        prog_factory, _ = stubbed_progressive_service
        qcm_factory("wrong")
        prog_factory("full_after_attempts")
        result = runner.invoke(
            app,
            [
                "submit-qcm",
        ...
        for i in range(start, len(text)):
            ...
            if ch == "{":
                ...
                if depth == 0:
                    ...
                    break
        payload = json.loads(text[start:end])
        assert payload["correction_level"] == "full_after_attempts"
        assert payload["solution"] == "solution complete"

    def test_submit_qcm_fourth_try_returns_six(
        self, stubbed_qcm_grader, stubbed_progressive_service
    ) -> None:
        """AC9 — 4th attempt → closed → exit 6 (new exit code)."""
        qcm_factory, _ = stubbed_qcm_grader
        prog_factory, _ = stubbed_progressive_service
        qcm_factory("wrong")
        prog_factory("closed")
        ...
        # The QCM grader must NOT have been called — the closed gate
        # is enforced by the service, not the CLI.
        assert qcm_factory is not None  # type sanity

    def test_submit_qcm_partial_attempt_2_returns_partial_attempt_2(
        self, stubbed_qcm_grader, stubbed_progressive_service
    ) -> None:
        """AC2 — second failure → correction_level='partial_attempt_2'."""
        qcm_factory, _ = stubbed_qcm_grader
        prog_factory, _ = stubbed_progressive_service
        qcm_factory("wrong")
        prog_factory("partial_attempt_2")
        result = runner.invoke(
            app,
            [
                "submit-qcm",
        ...
        for i in range(start, len(text)):
            ...
            if ch == "{":
                ...
                if depth == 0:
                    ...
                    break
        payload = json.loads(text[start:end])
        assert payload["correction_level"] == "partial_attempt_2"
        assert payload["hints"] == ["erreur identifiee"]


class TestSubmitTextProgressive:
    """s08 — submit-text is extended to call the progressive service."""

    def test_submit_text_first_try_failure_returns_partial(
        self, stubbed_text_grader, stubbed_progressive_service
    ) -> None:
        """AC1 — text first failure → partial."""
        text_factory, _ = stubbed_text_grader
        prog_factory, holder = stubbed_progressive_service
        text_factory("echec")
        prog_factory("ok")
        result = runner.invoke(
        ...
        assert result.exit_code == EXIT_OK, result.stdout
        assert len(holder["svc"].calls) == 1
        # The text grader reported failure and the service
        # translated that into (is_success=False, ...).
        assert holder["svc"].calls[0][2] == (False, "Réponse incomplète.")
        text = result.stdout
        start = text.find("{")
        depth = 0
        end = -1
    ...
    def test_submit_text_fourth_try_returns_six(
        self, stubbed_text_grader, stubbed_progressive_service
    ) -> None:
        """AC9 — text 4th attempt → closed → exit 6."""
        text_factory, _ = stubbed_text_grader
        prog_factory, _ = stubbed_progressive_service
        text_factory("echec")
        prog_factory("closed")
    ...
    def test_submit_text_invalid_exercise_returns_4(
        self, stubbed_text_grader, stubbed_progressive_service
    ) -> None:
        """D10 — FLASHCARDS via the progressive service → exit 4 (bad input)."""
        text_factory, _ = stubbed_text_grader
        prog_factory, _ = stubbed_progressive_service
        text_factory("echec")
        prog_factory("invalid_exercise")
        result = runner.invoke(
            app,
            [
                "submit-text",
